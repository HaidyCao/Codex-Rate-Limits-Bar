...
- Local snapshots report cache persistence as `saved`, `pending`, or `disabled`. Failed saves remain visible and retry on later refreshes, including when logs are unchanged. Verified totals remain available. See [cache persistence and recovery](docs/cache-persistence.md).
